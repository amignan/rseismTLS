...
The model parameters are estimated with `model_par.mle()` by minimizing the negative log-likelihood function `negloglik.val()`, as given in Broccardo et al. (2017:A2):

```{r}
data <- list(seism = seism, inj = inj, m0 = mc, ts = t.shutin, Tmax = 12)
(par.MLE <- rseismTLS::model_par.mle(data))
```
...
```{r, fig.show='hold', fig.path='figs/'}
rate.coinj.pred <- rseismTLS::model_rate.val('injection', 
...
It should be noted that in the case in which only the seismicity rate is available (`seism.binned`), the model parameters can be estimated using the Poisson log-likelihood function, assuming a Non-Homogeneous Poisson Process (NHPP). It is computed in `poi.loglik()` and the induced seismicity model log-likelihood by `ratemodel.loglik()`.
